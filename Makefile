.PHONY: test phpunit phpstan phpcs phpcbf phpcbf-all help
...
help:
	...
	@echo ""
	@echo "Targets:"
	@echo "  test        Run all checks (phpstan + phpcs + phpunit)"
...
# Run all checks in the app container
test:
	docker compose run --rm shipmonk-packing-app bash -lc "composer phpstan && composer phpcs && ./vendor/bin/phpunit"
